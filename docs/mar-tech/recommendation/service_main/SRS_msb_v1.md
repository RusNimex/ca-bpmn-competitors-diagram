...
Модуль входит в систему рекомендаций конкурентов. Источник — выгрузка 2GIS (CSV, UTF-8). Данные идут в отдельную базу МСБ и используются стратегией 3 (algo=MSB) — [service_rest/SRS_v2.md](../service_rest/SRS_v2.md).
...
| FR-005 | Категория >532 символов → обрезка по последней запятой; лог «битые категории» + отображение на странице |
...
- **NFR-002** — Прогресс обновляется достаточно часто (комфортно для пользователя)
...
| **Битые категории** | Обрезка при >532 символов (FR-005) |
